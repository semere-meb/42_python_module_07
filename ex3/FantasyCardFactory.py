...
class FantasyCardFactory(CardFactory):
    """A factory that creates fantasy-themed cards."""

    def __init__(self) -> None:
        self._creature_templates = {
            "dragon": {"name": "Fire Dragon", "cost": 5, "rarity": "Legendary", "attack": 5, "health": 5},
            "goblin": {"name": "Goblin Warrior", "cost": 2, "rarity": "Common", "attack": 2, "health": 2},
        }

        self._spell_templates = {
            "fireball": {"name": "Fireball", "cost": 3, "rarity": "Rare", "effect_type": "fire"},
            "lightning": {"name": "Lightning Bolt", "cost": 3, "rarity": "Rare", "effect_type": "lightning"},
        }

        self._artifact_templates = {
            "mana_ring": {"name": "Mana Ring", "cost": 2, "rarity": "Uncommon", "durability": 3, "effect": "mana boost"},
            "ancient_staff": {"name": "Ancient Staff", "cost": 4, "rarity": "Epic", "durability": 5, "effect": "spell power"},
        }

    def create_creature(self, name_or_power: str | int | None = None) -> CreatureCard:
        if isinstance(name_or_power, str) and name_or_power.lower() in self._creature_templates:
            template = self._creature_templates[name_or_power.lower()]
        elif isinstance(name_or_power, int):
            # Choose a creature with similar cost if possible
            template = min(
    ...
    def create_spell(self, name_or_power: str | int | None = None) -> SpellCard:
        if isinstance(name_or_power, str) and name_or_power.lower() in self._spell_templates:
    ...
    def create_artifact(self, name_or_power: str | int | None = None) -> ArtifactCard:
        if isinstance(name_or_power, str) and name_or_power.lower() in self._artifact_templates:
    ...
    def create_themed_deck(self, size: int) -> dict:
        """Create a small themed deck for a game turn."""
        types = ["creature", "spell", "artifact"]
        deck: dict[str, list[Any]] = {"creatures": [], "spells": [], "artifacts": []}
